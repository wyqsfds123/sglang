...
class WeightDiskUpdateController:
    ...
    async def update_weights_from_disk(
        ...
        async with lock_context:
            success, message, num_paused_requests = (
                await WeightDiskUpdateController._wait_for_model_update_from_disk(
                    self, obj
                )
            )

        if success and obj.weight_version is not None:
            WeightDiskUpdateController._update_weight_version_if_provided(
    ...
    async def _wait_for_model_update_from_disk(
        ...
        if self.config.dp_size == 1:
            ...
            if result.success:
                WeightDiskUpdateController._update_model_path_info(
