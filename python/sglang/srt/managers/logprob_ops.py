...
def fill_meta_info(
    ...
    *,
    ...
    if len(state.logprobs.input_token_logprobs_val) > len(state.logprobs.input_token_logprobs):
        state.logprobs.input_token_logprobs.extend(
            _detokenize_logprob_tokens(
                state.logprobs.input_token_logprobs_val[len(state.logprobs.input_token_logprobs) :],
                state.logprobs.input_token_logprobs_idx[len(state.logprobs.input_token_logprobs) :],
                decode_to_text=return_text_in_logprobs,
                tokenizer=tokenizer,
            )
        )

    if len(state.logprobs.output_token_logprobs_val) > len(state.logprobs.output_token_logprobs):
        state.logprobs.output_token_logprobs.extend(
            _detokenize_logprob_tokens(
                state.logprobs.output_token_logprobs_val[len(state.logprobs.output_token_logprobs) :],
                state.logprobs.output_token_logprobs_idx[len(state.logprobs.output_token_logprobs) :],
                decode_to_text=return_text_in_logprobs,
                tokenizer=tokenizer,
            )
        )
    ...
    meta_info["output_token_logprobs_length"] = len(state.logprobs.output_token_logprobs)

    # 2. Handle top logprobs
    if top_logprobs_num > 0:
        if len(state.logprobs.input_top_logprobs_val) > len(state.logprobs.input_top_logprobs):
            state.logprobs.input_top_logprobs.extend(
                _detokenize_top_logprobs_tokens(
                    state.logprobs.input_top_logprobs_val[len(state.logprobs.input_top_logprobs) :],
                    state.logprobs.input_top_logprobs_idx[len(state.logprobs.input_top_logprobs) :],
                    decode_to_text=return_text_in_logprobs,
                    tokenizer=tokenizer,
                )
            )
        if len(state.logprobs.output_top_logprobs_val) > len(state.logprobs.output_top_logprobs):
            state.logprobs.output_top_logprobs.extend(
                _detokenize_top_logprobs_tokens(
                    state.logprobs.output_top_logprobs_val[len(state.logprobs.output_top_logprobs) :],
                    state.logprobs.output_top_logprobs_idx[len(state.logprobs.output_top_logprobs) :],
                    decode_to_text=return_text_in_logprobs,
                    tokenizer=tokenizer,
                )
            )
