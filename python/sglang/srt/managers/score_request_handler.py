...
class ScoreRequestHandler:
    ...
    async def score_prompts(
        ...
        if isinstance(prompts, str) or (
            ...
            return await ScoreRequestHandler.score_request(
                self,
                query="",
    ...
    def _process_multi_item_scoring_results(
        ...
        if input_logprobs:
            ...
            for logprobs_data in input_logprobs:
                logprobs = ScoreRequestHandler._extract_logprobs_for_tokens(
                    self, logprobs_data, label_token_ids
                )
                score_list = ScoreRequestHandler._convert_logprobs_to_scores(
    ...
    def _resolve_embed_overrides_for_request(
        ...
        """
        q_embeds, q_positions = ScoreRequestHandler._resolve_overrides_for_sequence(
            self,
            query,
        ...
        i_embeds, i_positions = ScoreRequestHandler._resolve_overrides_for_sequence(
            self,
            item,
            item_embeds,
            embed_override_token_id,
    ...
    async def score_request(
        ...
        if use_text_prompts:
            ...
            query_ids, items_ids = ScoreRequestHandler._batch_tokenize_query_and_items(
                self, query, items
            )
            _, input_ids, positional_embed_overrides, delimiter_indices = (
                ScoreRequestHandler._build_token_id_inputs(
        ...
        if use_multi_item_scoring:
            ...
            return ScoreRequestHandler._process_multi_item_scoring_results(
                self,
                results,
                items,
                label_token_ids,
            ...
            return ScoreRequestHandler._process_single_item_scoring_results(
                self,
                results,
                label_token_ids,
                apply_softmax,
